...
static std::mutex g_emailMutex;
static std::chrono::steady_clock::time_point g_lastEmailTime = std::chrono::steady_clock::now();
    ...
    if (!EvtRender(NULL, hEvent, EvtRenderEventXml, dwBufferSize, pBuffer, &dwBufferUsed, &dwPropertyCount)) {
        if (GetLastError() == ERROR_INSUFFICIENT_BUFFER) {
            ...
            if (EvtRender(NULL, hEvent, EvtRenderEventXml, dwBufferUsed, pBuffer, &dwBufferUsed, &dwPropertyCount)) {
                ...
                // LogonType 7 = 解锁工作站：重新连接到已断开的会话
                if ((logonType == L"2" || logonType == L"7" || logonType == L"10") &&
                    userName != L"SYSTEM" &&
                    userName != L"DWM-1" &&
                    ...
                    } else if (logonType == L"7") {
                        sLoginTypeDesc = "RDP 会话恢复";
                    }

                    // 彻底抛弃 cout，使用安全的 log() 函数
                    ...
                    if (SMTP_FEATURE_ENABLED && EVERY_LOGIN_PROCEED_AN_EMAIL) {

                        std::lock_guard<std::mutex> lock(g_emailMutex);

                        auto now = std::chrono::steady_clock::now();
                        auto duration = std::chrono::duration_cast<std::chrono::seconds>(now - g_lastEmailTime).count();

                        // 设定冷却时间为 15s
                        if (duration >= 15) {
                            send_email(1, WStringToString(userName), WStringToString(ExtractIpAddress(xmlStr)));
                            g_lastEmailTime = now; // 更新发送时间
                        } else {
                            log(mainProgram, "UserSecurityHandler::ProcessSuccessEvent",
                                "[INFO] 距离上次发邮件仅过去 %lld s。为避免邮件轰炸以及开发人员的 SMTP 被禁用，本次发送跳过。", duration);
                        }
                    }
                }
            } else {
